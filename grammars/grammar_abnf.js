...
function CreateAbnfGrammar(myna) {
    ...
    let g = new function() {		
		...
		this.prose_val      =  m.tagged(m.notChar('>').zeroOrMore); /* bracketed string of SP and VCHAR without angles prose description, to be used as last resort */
		...
		this.num_val        =  m.seq("%", m.choice(this.bin_val, this.dec_val, this.hex_val));
		// char-val       	=  DQUOTE *(%x20-21 / %x23-7E) DQUOTE
		this.char_val       =  m.doubleQuoted(m.notChar('"').oneOrMore); /* quoted string of SP and VCHAR without DQUOTE */
		...
		this.repetition     =  m.seq(this.repeat.opt, this.element);
